...
std::string TrimSpaces(const std::string& s)
{
	...
	{
		str.erase( 0, 1);
		a = str.find_first_of(' ', 0 );
	}
	int b = str.find_last_of(' ', -1 );
	while( b == str.length()-1 && b != -1)
	{
		str.erase(b);
...
bool CIniFile::ReadFile()
{
  ...
  while( getline( f, line)) {
	...
	case '=':
	  ...
	  valuename = TrimSpaces(valuename);
...
string CIniFile::GetValue( unsigned const keyID, unsigned const valueID) const throw (IniValueException)
{
  if ( keyID < keys.size() && valueID < keys[keyID].names.size())
    return keys[keyID].values[valueID];
  throw IniValueException(string());
  return string();
}

string CIniFile::GetValue( string const keyname, string const valuename) const throw (IniValueException)
{
  long keyID = FindKey( keyname);
  if ( keyID == noID)
    throw IniValueException(string());
  ...
  std::string s = keys[keyID].values[valueID];
  if(s.empty()) throw EmptyValueException(string());
  return s;
}

int CIniFile::GetValueI(string const keyname, string const valuename) const throw (IniValueException)
{
	const char* str = GetValue( keyname, valuename).c_str();
	char *ch = 0;
	long i = /*atoi( str)*/strtol(str, &ch, 10); 
	if(*ch) throw InvalidValueException(valuename);
	return i;
}

double CIniFile::GetValueF(string const keyname, string const valuename) const throw (IniValueException)
{
	const char* str = GetValue( keyname, valuename).c_str();
	char *ch = 0;
	long i = strtod(str, &ch); 
	if(*ch) throw InvalidValueException(valuename);
	return i;
}
