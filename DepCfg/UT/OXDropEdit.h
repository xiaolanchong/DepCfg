...
protected:
	...
	// the button with 'Alt' key pressed in.
...
public:
	...
	inline void SetButtonWidth(const int nButtonWidth) { 
	...
	inline void ShowButton(const BOOL bShowButton) {
	...
	//				button  in the control
	inline void SetButtonAlignment(const int nButtonAlignment) { 
	...
	// --- In  :	
	// --- Out : 
	// --- Returns: Virtual-key code of the button that can be used in combination with 
	//				pressed in 'Alt' key in order to emulate clicking on the dropdown 
	//				button
	// --- Effect : Retrieves the virtual-key code of the button that can be used in 
	//				combination with pressed in	'Alt' key in order to emulate clicking
	//				on the dropdown button
	inline int GetDropdownKey() const { return m_nVCDropdownKey; }
											...
											LPARAM lParam) 
{
	...
	case WM_SYSKEYDOWN:
		{
			LRESULT lResult=PARENTEDIT::WindowProc(message, wParam, lParam);
			if((int)wParam==GetDropdownKey() && (GetKeyState(VK_MENU)<0))
			{
				if(m_bButtonPressed)
				{
					m_bButtonPressed=FALSE;
	...
	return PARENTEDIT::WindowProc(message, wParam, lParam);
}
