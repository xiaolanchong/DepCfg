...
public:
	...
	BOOL AddPage(CRuntimeClass* pClass, CCreateContext* pContext, 
		LPCTSTR lpszTitle=NULL, const int nImage=-1) {
	...
	virtual BOOL InsertPage(const int nIndex, CRuntimeClass* pClass, 
		CCreateContext* pContext, LPCTSTR lpszTitle=NULL, const int nImage=-1);
	...
	//				function if you have to add any generic window into the 
	//				3DTabView container.
	virtual BOOL InsertPage(const int nIndex, CWnd* pWnd, 
		LPCTSTR lpszTitle=NULL, const int nImage=-1);
	...
	// --- Returns: image index of the corresponding page
	// --- Effect : Retrieves the image index of the specified page
	inline int GetPageImageIndex(const int nIndex) const {
		ASSERT(nIndex>=0 && nIndex<GetPageCount());
		if(nIndex>=0 && nIndex<GetPageCount())
		{
			TCITEM tci;
	...
	// --- Returns: 
	// --- Effect : Sets the image index of the specified page
	inline BOOL SetPageImageIndex(const int nIndex, int nImage) {
		ASSERT(nIndex>=0 && nIndex<GetPageCount());
		if(nIndex>=0 && nIndex<GetPageCount())
		{
			TCITEM tci;
	...
	inline void SetOffsetExternal(const DWORD dwOffset) 
	{ 
	...
	//				display rectangle
	inline void SetOffsetInternal(const DWORD dwOffset) 
	{ 
