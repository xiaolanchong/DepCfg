...
BOOL COX3DTabViewContainer::InsertPage(const int nIndex, 
...
BOOL COX3DTabViewContainer::InsertPage(const int nIndex, CWnd* pWnd, 
									   LPCTSTR lpszTitle/*=NULL*/, 
									   const int nImage/*=-1*/)
{
	ASSERT_VALID(this);
...
//	pWnd->ModifyStyleEx(NULL,WS_EX_CLIENTEDGE);
	pWnd->ModifyStyle(NULL,WS_BORDER);
	SetActivePageIndex(nIndex);

	return TRUE;
}

BOOL COX3DTabViewContainer::DeletePage(const int nIndex, 
...
BOOL COX3DTabViewContainer::SetActivePageIndex(const int nIndex)
{
